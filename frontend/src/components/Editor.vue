          ...
          <div v-else key="stats" class="flex flex-wrap items-end h-full">
            <div class="px-3 w-full lg:w-5/12">
              <div v-if="stats.savings > 0">
                <h2
                  class="font-bold leading-none text-4xl md:text-5xl text-pink-default tracking-tight"
                >
          ...
          <tbody>
            <tr v-for="(file, i) in files" :key="`${i}-${file.name}`">
              <td
                class="cell-l ta"
                :class="{
                  'text-gary-400': !file.isProcessed,
                  'text-gary-200': file.isProcessed,
              ...
              <td :class="{ 'anime-txt-success': file.isConverted }">
...
export default defineComponent({
  ...
  mounted() {
    ...
    Wails.Events.On("conversion:stat", e => {
      ...
      this.stats.count += c;
      this.stats.time += t;
      if (s > 0) this.stats.savings += e.savings;
